...
test -e ${PREFIX}.fasta.chr.rename.txt || die "File ${PREFIX}.fasta.chr.rename.txt does not exist. Did you run minidotplot.sh?"
test -e ${PREFIX}/${PREFIX}.windows.0.4.50kb.ends.bed || die "File ${PREFIX}/${PREFIX}.windows.0.4.50kb.ends.bed does not exist. Did you run telostat.sh?"

awk '{print "s/"$1"/"$2"/g"}' ${PREFIX}.fasta.chr.rename.txt | sed -f - ${PREFIX}/${PREFIX}.windows.0.4.50kb.ends.bed | sort -k1,1 | cut -f1 | uniq -c | sort -k1,1 -r -n | awk '{print $2"\t"$1}' > $PREFIX.fasta.fix.tmp.telostat.txt
...
do
T2T=$(grep $chr"_" $PREFIX.fasta.fix.tmp.telostat.txt | awk '{if($2==2){print "y"} else {print "n"}}' |  tr '\n' ',')
NTelo=$(grep $chr"_" $PREFIX.fasta.fix.tmp.telostat.txt | datamash sum 2)
cat $PREFIX.fasta.fix.tmp.telostat.txt  | grep $chr"_" | cut -f 1 > $PREFIX.fasta.fix.tmp.telostat.$chr.txt || die "Error in extracting telomere contigs"
TELLEN=$(cat $FILE | cut -f 1,2 | grep -w -F -f $PREFIX.fasta.fix.tmp.telostat.$chr.txt | uniq -c | cut -f 2 | tr '\n' ',')
echo -e $chr"\t"$T2T"\t"$NTelo"\t"$TELLEN
done


echo "Contigs whose majority is mapped to the corresponding chromosome"
echo "\tNcontigsofsize>=KMbasealignedtochr\t\t\t\t\t%ofchrsequencecoveredbycontigsofsize>=KMbase				"
