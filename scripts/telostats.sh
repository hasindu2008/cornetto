...
${CORNETTO} telofind $FILE | awk '{print $1"\t"$(NF-4)"\t"$(NF-3)"\t"$(NF-2)"\t"$(NF-1)"\t"$NF}' - > $TEMPDIR/$PREFIX.telomere || die "cornetto telofind failed"
${CORNETTO} fa2bed $FILE | awk '{print $1"\t"$3}' > $TEMPDIR/${PREFIX}.lens || die "fa2bed failed"
...
cat $TEMPDIR/$PREFIX.lens | awk -v ends=$ENDS '{if ($2>(ends*2)) {print $1"\t0\t"ends"\n"$1"\t"($NF-ends)"\t"$NF} else {print $1"\t0\t"$NF}}' > $TEMPDIR/asm.ends.bed || die "awk failed"
